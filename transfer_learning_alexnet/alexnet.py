...
class AlexNet(object):
    ...
    def create(self):
        ...
        connect_fc = fc(dropout6, self.data_batch_size, 1, name='fc_connect', transpose=True)
        print('Connect fc shape: ', connect_fc.get_shape().as_list())

        # Squeeze dropout layer from Task network and duplicate to match data batch size
        task_embedding_tile = tf.reshape(tf.tile(connect_fc, [1, self.data_batch_size]),
                                         [self.data_batch_size, -1])

        print('Tile shape: ', task_embedding_tile.get_shape().as_list())

        dropout_combined = tf.concat([dropout6, task_embedding_tile], 1)

        print('Combined dropout shape: ', dropout_combined.get_shape().as_list())
...
def fc(x, num_in, num_out, name, relu=True, transpose=False):
    if transpose:
        x = tf.transpose(x)
