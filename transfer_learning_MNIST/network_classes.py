...
class TaskEmbeddingNetworkNaive:
    ...
    def _create(self):
        ...
        with tf.variable_scope('input_network'):
            ...
            self.fc_fnl = fc_layer_naive(self.inp_fc1, 10, name='fc_fnl', non_linear_fn=None)
...
class SingleGraph:
    ...
    This is the graph for single task training
    """
    def __init__(self, hidden_layers=None, input_features_dim=784, learning_rate=0.01, weight_decay=0.0005):
        self._learning_rate = learning_rate
    ...
    def _create(self):
        with tf.variable_scope('single_graph'):
            ...
            self.fc_fnl = fc_layer_naive(getattr(self, 'fc{}'.format(len(self._hidden_layers) - 1)),
                                         10, name='fc_fnl', non_linear_fn=None)
            self.pred = tf.argmax(tf.nn.softmax(self.fc_fnl), 1)
