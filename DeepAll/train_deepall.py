...
data_generator_train = ImageDataGenerator(data_dir + 'train_PACS.txt', task_sequence, data_batch_size,
                                          no_of_tasks, num_classes, data_dir, test_task=test_task)
...
# Initialize model
model = AlexNet(data_x, keep_prob, num_classes, train_layers,
...
with tf.Session() as sess:
    ...
    with open(log_file, 'w+') as log_f:
        ...
        data_generator_test = TestDataGenerator(data_dir + 'train_PACS.txt', task_sequence, data_batch_size,
                                                no_of_tasks, num_classes, data_dir,
                                                test_task=test_task)
        for epoch in range(num_epochs):
            ...
            data_batch, label_batch = next(random_iterator_train)
            ...
            if epoch % display_step == 0:
                ...
                for itr in range(batches_per_itr):
                    # get next batch of data
                    data_batch, label_batch = next(random_iterator_test)
                    batch_one_hot = sess.run(tf.one_hot(label_batch, num_classes))
