...
class TaskEmbeddingNetwork:
    ...
    """
    def __init__(self, learning_rate=0.01, input_features_dim=2, task_embedding_layers=[2], input_network_layers=[2]):
    ...
    def _create(self):
        self.task_batch = tf.placeholder(tf.float32, shape=[None, self._input_dim])
        ...
        with tf.variable_scope('task_embedding_network'):
            self.task_fc0 = fully_connected_layer(self.task_batch, self._task_embedding_layers[0], name='fc0')
        ...
        with tf.variable_scope('input_network'):
            self.input = tf.concat([self.input_batch, self.task_embedding], axis=1)
            ...
            self.fc_fnl = fully_connected_layer(getattr(self, 'inp_fc{}'.format(len(self._input_network_layers) - 1)),
                                                  name='fc_fnl', non_linear_fn=None)
