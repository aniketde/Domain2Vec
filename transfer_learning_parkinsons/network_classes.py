...
class TaskEmbeddingNetworkNaive:
    ...
    def _create(self):
        ...
        with tf.variable_scope('input_network'):
            ...
            self.fc_fnl = fc_layer_naive(self.inp_fc1, 1, name='fc_fnl', non_linear_fn=None)
        self.pred = self.fc_fnl#tf.argmax(tf.nn.softmax(self.fc_fnl), 1)
    ...
    def _train(self, sess, iterator, epochs, num_samples, ckpt_check=False):
        ...
        with tf.variable_scope('optimization'):
            ...
            self.losses = tf.reduce_mean(tf.nn.l2_loss(self.output - self.fc_fnl))
    ...
    def predictions(self, sess, test_iterator, test_tasks, num_samples):
        ...
        for i in range(test_tasks * (task_batch.shape[0]//labels_batch.shape[0])):
            ...
            pred_vector[j:(j+labels_batch.shape[0])] = prediction[:,0]
            total_loss += loss
...
class TaskEmbeddingNetwork:
    ...
    This is the graph for single task training
    """
    def __init__(self, learning_rate=0.01, input_features_dim=19, task_embedding_layers=[2], task_batch_size=1024, data_batch_size=256, input_network_layers=[2]):
        self._learning_rate = learning_rate
        self._input_dim = input_features_dim
...
class SingleGraph:
    ...
    This is the graph for single task training
    """
    def __init__(self, hidden_layers=None, input_features_dim=19, learning_rate=0.01, weight_decay=0.0005):
        self._learning_rate = learning_rate
        if hidden_layers is None:
            self._hidden_layers = [2]
        else:
    ...
    def _train(self, sess, iterator, epochs, num_samples):
        # Getting the basic variables required to run loops for the desired number of epochs
        _, data, y = next(iterator)
        ...
        with sess.as_default():
            ...
            for step in range(num_cycles - global_step.eval()):
                ...
                _, data, y = next(iterator)
                if (step * batch_size) % num_samples == 0:
                    print("Epoch {} : Training Loss = {}, Accuracy: {}".format((step * batch_size) // num_samples,
                                                                               total_loss, accuracy))

    def predictions(self, sess, test_data, test_outputs):
        prediction, total_loss, accuracy = sess.run([self.pred, self.losses, self.accuracy], feed_dict={
            self.input_ph: test_data, self.output: test_outputs})
        return prediction, total_loss, accuracy
